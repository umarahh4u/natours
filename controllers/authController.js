...
exports.protect = catchAsync(async (req, res, next) => {
  // 1) Getting token and check if't there
  let token;
  if (
  ...
  if (!token) {
    return next(
      new AppError('Your are not loggged in! Please log in to get access', 401)
    );
  }
...
// req.secure does not work at the first place, we need to add trust proxy in app.js to work
